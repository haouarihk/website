...
const calculateLicensePrice = (
	...
	const price = isAnnual ? prices[type].annual : prices[type].monthly;
	return price * count;
};
...
export function Pricing() {
	...
	const [basicServers, setBasicServers] = useState(1);
	const [professionalServers, setProfessionalServers] = useState(1);
	const [enterpriseServers, setEnterpriseServers] = useState(1);
	const [isLoading, setIsLoading] = useState(false);
	const featured = true;

	const handleBuyNow = async (plan: "basic" | "premium" | "business") => {
		try {
			setIsLoading(true);

			// Get server quantity based on plan
			const servers = {
				basic: basicServers,
				premium: professionalServers,
				business: enterpriseServers,
			}[plan];
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
								...
								<section className="flex flex-col rounded-3xl border-2 border-dashed border-muted px-4 py-8">
									...
									<div className="mt-4 flex flex-col gap-2">
										<div className="flex items-center justify-center gap-2">
											<span className="text-sm text-muted-foreground">
												{t("plan.selfHosted.servers", {
													serverQuantity: basicServers,
												})}
											</span>
										</div>
										<div className="flex items-center space-x-2">
											<Button
												disabled={basicServers <= 1}
												variant="outline"
												onClick={() => {
													if (basicServers <= 1) return;
													setBasicServers(basicServers - 1);
												}}
											>
												<MinusIcon className="h-4 w-4" />
											</Button>
									...
									<div className="mt-6 flex flex-col gap-2">
										<Button
											...
											disabled={isLoading}
										>
											{isLoading ? "Loading..." : t("plan.selfHosted.buyNow")}
										</Button>
									</div>
								</section>
								...
								<section className="flex flex-col rounded-3xl border-2 border-dashed border-muted px-4 py-8 bg-black/50">
									...
									<div className="mt-4 flex flex-col gap-2">
										<div className="flex items-center justify-center gap-2">
											<span className="text-sm text-muted-foreground">
												{t("plan.selfHosted.servers", {
													serverQuantity: professionalServers,
												})}
											</span>
										</div>
										<div className="flex items-center space-x-2">
											<Button
												disabled={professionalServers <= 1}
												variant="outline"
												onClick={() => {
													if (professionalServers <= 1) return;
													setProfessionalServers(professionalServers - 1);
												}}
											>
												<MinusIcon className="h-4 w-4" />
											</Button>
											<NumberInput
												value={professionalServers}
												onChange={(e) => {
													setProfessionalServers(
								...
								<section className="flex flex-col rounded-3xl border-2 border-dashed border-muted px-4 py-8">
									...
									<div className="mt-4 flex flex-col gap-2">
										<div className="flex items-center justify-center gap-2">
											<span className="text-sm text-muted-foreground">
												{t("plan.selfHosted.servers", {
													serverQuantity: enterpriseServers,
												})}
											</span>
										</div>
										<div className="flex items-center space-x-2">
											<Button
												disabled={enterpriseServers <= 1}
												variant="outline"
												onClick={() => {
													if (enterpriseServers <= 1) return;
													setEnterpriseServers(enterpriseServers - 1);
												}}
											>
												<MinusIcon className="h-4 w-4" />
											</Button>
											<NumberInput
												value={enterpriseServers}
												onChange={(e) => {
													setEnterpriseServers(
														e.target.value as unknown as number,
													);
												}}
											/>
