...
install_dokploy() {
    ...
    # 🔹 Syncthing service to sync /etc/dokploy/traefik across all managers
    docker service create \
    --name dokploy-traefik-sync \
    --mode global \
    --constraint 'node.role==manager' \
    --network dokploy-network \
    --mount type=bind,source=/etc/dokploy/traefik,target=/var/syncthing/traefik \
    syncthing/syncthing:latest \
    syncthing -home /var/syncthing/config -gui-address="0.0.0.0:8384" -no-browser
    
    docker network connect dokploy-network dokploy-traefik-sync
